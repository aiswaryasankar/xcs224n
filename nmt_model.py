...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(input_size=embed_size,
                               ...
                               bias=True,
                               bidirectional=True)  # (input, h_0, c_0)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # compute the model embeddings of the source sentences
        X = self.model_embeddings.source(source_padded)
        X = torch.nn.utils.rnn.pack_padded_sequence(X, lengths=source_lengths)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)
        enc_hiddens, _ = torch.nn.utils.rnn.pack_packed_sequence(enc_hiddens)[0].transpose(0,1)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for y_t in torch.split(Y, 1, dim=0):
            ...
            Y_hatT = torch.cat((squeezed, o_prev), dim=1)
            dec_state, o_t, _ = self.step(Y_hatT, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = enc_hiddens_proj.bmm(dec_hidden.unsquuze(2)).squeeze(2)
        ### END YOUR CODE
